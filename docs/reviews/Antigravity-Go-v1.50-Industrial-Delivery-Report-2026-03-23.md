...
- 启动自愈补齐：`cmd/agy/main.go` 增加环境自检；检测到配置文件损坏或数据目录异常时，显式阻断并提供 `--safe-start` / `--auto-repair` 两条恢复路径。
