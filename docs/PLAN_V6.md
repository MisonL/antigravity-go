...
- `ago init` 已实现，能够在空目录生成极简标准的 `Go 后端 + React/Vite 前端` 脚手架，并落地 `Makefile`、`README.md` 与基础工程文件。
...
- 当前前端执行 `npm run build` 通过，且已消除 Vite 的大体积分块警告。
...
- 控制器:
  `agent.Agent`、生成者-审查者、审查专家、TUI/Web 交互层、`ago review`/`ago auto-fix`/`ago deploy`/`ago init`。
- 执行器:
  `apply_core_edit`、`write_file`、`rollback_to_step`、浏览器工具、MCP 动态工具、部署记录写入。

### 1.2 结论

当前系统不是“没有闭环”，而是“局部闭环存在、全局闭环不完整”。

- 观测器层已经具备基础观测能力，但传感器分散，缺少统一任务级状态面。
- 控制器层已经能做补丁、自检、预审和审批，但没有项目级任务账本和长期控制策略。
- 执行器层覆盖代码、浏览器、MCP、回滚与部署，但没有形成一致的策略编排面。

### 1.3 当前单点故障

- `antigravity_core` 是明显单点故障。`internal/core/host.go` 只管理单个核心进程，内核挂掉时 TUI、Web、Agent 与 MCP 全部失效。
- `WSServer` 是单控制面汇聚点。单个 WebSocket 中枢负责会话、审批、广播和观测事件，失效会导致 Web 交互面整体失联。
- LLM Provider 是单推理通道。当前没有 模型服务级降级编排、熔断和任务重放机制。
...
2. 缺少 supervisor 级自恢复。
   现状：内核、WebSocket、模型服务 失败后主要依赖人工重试。
   后果：控制器本身没有对 被控对象 做持续稳定控制。
   要求：引入 host 看门狗、模型服务重试预算、Web 会话重连状态机。

3. 观测存在，但没有门禁级评分板。
   现状：有日志、摘要、validation、test output，但没有统一任务健康度、失败分类、时延与成功率面板。
   后果：难以判断系统是在收敛还是振荡。
   要求：建立 任务健康度、工具耗时、审批耗时、回滚次数、内核重连次数 等指标。
...
1. 多 Agent 并行编排。
   当前只有 `ask_specialist` 的轻量专家模式，没有真正的并行 工作单元、依赖图拆解和结果汇总能力。

2. 明确的任务规划与执行账本。
   Claude Code/Codex 的强项之一是把计划、执行、验证串成可追踪链路；当前系统仍然更像“带工具的聊天界面”。

3. 统一的变更审阅面。
   当前虽然已有 生成者-审查者 和审批预览，但还没有完整的“计划 diff -> 实际 diff -> 风险 diff -> 回滚点”同屏工作流。

4. 真正的断点续跑。
   当前 `resume` 已能恢复轨迹和消息，但还不能完整恢复环境变量、命令上下文、局部工作树状态和未完成子任务图。

5. 自动策略分层。
   当前 模型服务、工具、审批和回滚逻辑已存在，但缺少按任务类型自动切换执行策略的“上层大脑”。

### 3.2 建议优先补齐的能力

- 优先级 P0： 任务账本 + 任务级状态机。
- 优先级 P1： 并行 工作单元 + 审查者汇总。
- 优先级 P1： 以差异为中心的 审批面与回滚点对照。
- 优先级 P1： 可重放的 会话快照。
- 优先级 P2： 模板市场与项目类型选择器，而不是单一脚手架。
...
- 本轮虽然通过 `手动分块` 消除了构建 警告，但 构建包拆分仍是被动止损，不是系统性的前端架构收敛。
- 当前界面有明显“功能堆叠”感：轨迹、记忆、MCP、视觉自测、设置、聊天、终端、日志都在同一主界面竞争注意力。

### 4.3 UI/UX 行动项

- 优先级 P0： 拆分 `App.tsx`，至少分出 shell、chat、observability、settings、会话恢复 五个子域。
- 优先级 P1： 引入 通知中心，替换 `alert`。
- 优先级 P1： 建立 设计令牌 与组件级 CSS 边界，压缩 `index.css`。
- 优先级 P1： 重新设计 TUI 的审批、计划和执行三区布局。
- 优先级 P2： 为 Web 控制台补齐响应式断点和低带宽模式。
...
- 当前没有 语言包 文件、message key、语言切换、默认语言策略，也没有服务端与前端共享词条层。
...
- 优先级 P0： 建立统一文案层，前后端都改为 message key + 语言包 bundle。
- 优先级 P1： 默认提供 `zh-CN` 与 `en-US` 两套语言包。
- 优先级 P1： 错误码与用户文案分离，日志保留稳定英文 code，界面层再本地化。
- 优先级 P2： 对 提示词模板和专家提示词 引入可切换语言策略。

## 6. 分阶段补完清单

### 阶段 6A： 控制面收敛

- [已完成] 建立 `任务记录对象`，统一 任务引用、状态、证据、回滚点。
- [已完成] 为 内核宿主 增加 看门狗、重启策略、健康预算。
- [ ] 为 模型服务 增加错误分类、重试预算和 快速失败 策略。
- [已完成] 建立统一 可观测性记分板，而不只是散点摘要。

### 阶段 6B： 体验与工程拆分

- [已完成] 拆分 `frontend/src/App.tsx`。
- [已完成] 拆分 `frontend/src/index.css` 并建立 token 系统。
- [ ] 拆分 `internal/server/server.go` 与 `internal/server/websocket.go` 的协议职责。
- [ ] 重构 `internal/tui/model.go`，把输入、权限、渲染、命令总线拆开。

### 阶段 6C： 能力升级

- [已完成] 引入多 工作单元 并行执行与 审查者汇总。
- [ ] 为 `resume` 增加命令历史、环境快照、工作树差异恢复。
- [已完成] 建立 以差异为中心的 审批工作流。
- [ ] 扩展 `ago init` 为模板选择器，而不是单模板输出。

### 阶段 6D： 全球化与产品化

- [已完成] 建立 i18n 词条系统。
- [已完成] 清理 emoji 与装饰性输出，统一成纯文本与可翻译消息。
- [ ] 统一前端构建口径，选定 NPM 或 Bun，其余降为可选。
- [ ] 增加 CI 门禁，至少覆盖 `go build`、`go test ./...`、`frontend npm run build`。

### 阶段 6E： 最终完美化优化

- [已完成] 修正部署模板中的 `ENTRYPOINT` / `command` 叠加问题。
- [已完成] 为前端引入非阻塞通知中心并补齐窄屏布局收口。
- [已完成] 完成最终文本卫生清理与交付同步。
...
- 局部闭环已经成立。
- 统一任务状态面已经具备基础形态，但 模型服务策略分层仍待补齐。
- 单点故障仍然明显。
