...
func dockerComposeTemplate(plan DeploymentPlan) string {
	lines := []string{
		...
		"      - agy-data:/var/lib/agy",
		"      - ./deploy/runtime/antigravity_core:/opt/antigravity/bin/antigravity_core:ro",
		"    environment:",
		"      AGY_WEB_TOKEN: ${AGY_WEB_TOKEN:?set AGY_WEB_TOKEN before deployment}",
		...
		"      test:",
		"        - CMD-SHELL",
		"        - curl -fsS -H \"Authorization: Bearer $$AGY_WEB_TOKEN\" http://127.0.0.1:8888/api/status >/dev/null || exit 1",
		"      interval: 30s",
		"      timeout: 5s",
		"      retries: 5",
		"      start_period: 20s",
