...
func TestBuildDeploymentPlanForGoFrontendWorkspace(t *testing.T) {
	...
	assertContains(t, plan.DockerfileContent, "go build -trimpath -ldflags='-s -w' -o /out/agy ./cmd/agy")
	...
	assertContains(t, plan.DockerComposeBody, "curl -fsS -H \"Authorization: Bearer $$AGY_WEB_TOKEN\"")
