...
func (h *Host) WaitReady(timeout time.Duration) error {
	...
	// Active Probe: Try to connect to the port
	// The log says it's listening, but let's verify connectivity
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		// Try simple TCP connection first to see if port is open
		conn, err := net.DialTimeout("tcp", fmt.Sprintf("127.0.0.1:%d", port), 500*time.Millisecond)
		if err == nil {
			conn.Close()
			// Port is open and accepting attributes!
			h.mu.Lock()
			h.ready = true
			h.mu.Unlock()
			return nil
		}
		// Also check if existing ready flag was set via logs (fast path)
		h.mu.RLock()
		ready := h.ready
		h.mu.RUnlock()
		if ready {
	...
	return fmt.Errorf("timeout probing core port %d", port)
}
