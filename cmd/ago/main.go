// ago is the Antigravity-Go CLI Agent.
package main
...
func main() {
	// 子命令优先（符合设计文档：ago doctor/run/review/auto-fix/resume/mcp）
	...
	// Start Core Host
	fmt.Println("[INFO] 启动 Antigravity-Go (AGo)...")
	...
	if *noTUI {
		fmt.Println()
		fmt.Println("Antigravity-Go (AGo) 正在以无 TUI 模式运行")
		if coreReady {
