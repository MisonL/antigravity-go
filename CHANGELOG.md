...
### 新增
- 新增执行账本能力，在 CLI、TUI、HTTP API 和 Web 工作台之间统一记录执行摘要、时间线与派生步骤。
- 新增 `ago executions` 与 `ago execution <id>` 的 JSON 输出能力，并支持按 `status` 过滤执行记录。
- 新增基于 Bun 的 GitHub Actions 校验工作流，覆盖前端依赖安装、Lint、构建与后端测试。
- 新增一轮完整复核记录，覆盖内核兼容、Web 工作台、构建产物与接口验证。

### 变更
- 调整执行概览与可观测性面板，在 Web 工作台中补充执行详情弹窗、汇总统计和最近状态展示。
- 将 Web 工作台对外文案进一步产品化，移除 `MCP Services`、`Raw JSON`、`Target URL` 等工程化表达，并同步统一工作区、审批、视觉检查等界面说明。
...
- 修复工作台嵌入资源更新后部分状态消息、代码查看器与 MCP 面板文案不一致的问题。
